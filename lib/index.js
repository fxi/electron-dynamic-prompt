...
const electronPrompt = (options, parentWindow) => {
	return new Promise((resolve, reject) => {
		...
		let promptWindow = new BrowserWindow({
			...
			modal: options_.modal !== undefined ? options_.modal : Boolean(parentWindow),
			title: options_.title,
			icon: options_.icon || undefined,
			backgroundColor: options_.darkMode ? '#222' : '#fff',
			webPreferences: { ...(options_.webPreferences || {}), ...{
		...
		const postDataListener = (event, value) => {
			if(value == null) return cleanup()
			if(options_.validator !== undefined) {
				options_.validator(value)
				.then(() => {
					resolve(value)
					cleanup()
				})
				.catch(error => event.reply('prompt-post-data-reply:' + id, error))
			} else {
				resolve(value)
				cleanup()
			}
